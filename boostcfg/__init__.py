"""
The boostcfg parser is a Python module for parsing Boost config files that can be parsed by the
`parse_config_file
...
BoostConfigParser was especially designed for
...
class BoostConfigParser:
    ...
    def read_config(self, filename, reset=True):
        """Parse a config line and append key-value pairs to the internal dictionary"""
    ...
    def append_value(self, key, value):
        """
        Append a key-value pair to the internal dictionary: convert existing string values to a
        list if a new value with the same key is added
        """
